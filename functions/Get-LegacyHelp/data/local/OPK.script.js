...
// populateFbLink
// populates feedback links with CHM name and page title
function populateFbLink()
{	
	allAnchors = document.anchors;
	docLocation = document.URL;
	thisChmName = "";
	
	//checks for directory seperator chars and creates new pattern accordingly
	if(docLocation.indexOf("\\") > -1)
	{
		chmRegEx = new RegExp("^.*\\\\(.*?)::/.*$","i");
		chmRegEx.exec(docLocation);
		thisChmName = RegExp.$1;
	}

	
	//formats chm name if it has valid data (i.e.  that would be in a CHM name)
	if(thisChmName.match("\\w","i"))
	{
		thisChmName = thisChmName.toLowerCase() + "\\";
	}
	else
	{
		thisChmName = "";
	...
	//checks for fblink type anchors and applies feedback string
	for(i=0;i<allAnchors.length;i++)
	{
		if(allAnchors[i].id == "fbLink")
		{
			allAnchors[i].href = allAnchors[i].href + "?subject=FEEDBACK: " + thisChmName + document.title;
		}
	}
}
